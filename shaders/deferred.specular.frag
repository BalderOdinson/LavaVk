...
layout(input_attachment_index = 0, set = 0, binding = 0) uniform subpassInput in_depth;
layout(input_attachment_index = 1, set = 0, binding = 1) uniform subpassInput in_albedo;
layout(input_attachment_index = 2, set = 0, binding = 2) uniform subpassInput in_specular;
layout(input_attachment_index = 3, set = 0, binding = 3) uniform subpassInput in_normal;
...
vec3 cacluclate_directional_light(uint index, vec3 position, vec3 camera_position, vec3 norm, vec3 material_ambient, vec3 material_specular, vec3 material_diffuse, float shininess)
{
    ...
    vec3 specular_c = material_specular * light_specular * shininess;
    vec3 diffuse_c = max(dot(norm, light_dir), 0.0) * light_diffuse * material_diffuse;

    return ambient_c + specular_c + diffuse_c;
}

vec3 cacluclate_point_light(uint index, vec3 position, vec3 camera_position, vec3 norm, vec3 material_ambient, vec3 material_specular, vec3 material_diffuse, float shininess)
{
...
vec3 cacluclate_spotlight_light(uint index, vec3 position, vec3 camera_position, vec3 norm, vec3 material_ambient, vec3 material_specular, vec3 material_diffuse, float shininess)
{
    vec3 light_dir = normalize(lights.light[index].position.xyz - position);
    vec3 light_diffuse = lights.light[index].diffuse.rgb;
    vec3 light_ambient = lights.light[index].ambient.rgb;
...
void main()
{
    ...
    vec4 normal_data = subpassLoad(in_normal);
    ...
    else
    {
        ...
        for (uint i = 0; i < lights.count; ++i)
        {
            ...
            {
                result += cacluclate_directional_light(i, pos, vec3(ubo.inv_view[3]), normal, albedo.rgb, specular.rgb, albedo.rgb, specular.a * 255);
            }
            else if (lights.light[i].position.w == 1.f)
            {
                result += cacluclate_point_light(i, pos, vec3(ubo.inv_view[3]), normal, albedo.rgb, specular.rgb, albedo.rgb, specular.a * 255);
            }
            else if (lights.light[i].position.w == 2.f)
            {
                result += cacluclate_spotlight_light(i, pos, vec3(ubo.inv_view[3]), normal, albedo.rgb, specular.rgb, albedo.rgb, specular.a * 255);
            }
        }
